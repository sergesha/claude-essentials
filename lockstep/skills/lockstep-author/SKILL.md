...
| `review_verdict` | `path` **or** `path_from`; optional `expected: PASS\|FAIL` | Requires exactly one `Verdict: PASS` or `Verdict: FAIL` line and requires it to be the final non-whitespace content line. With `expected`, that exact verdict is required. | Use this instead of `file_matches` for review gates: an artifact containing conflicting or repeated verdict lines fails closed. It validates the stated verdict, not the quality of the review. |
