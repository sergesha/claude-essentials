...
NAMED and `_`-prefixed (inherits both anti-forgery
...
from collect-time project bytes**: the worker owns the project dir and the parent
...
- **Fractal child**: `artifact_hashes` are COPIED from the child run's
  ...
  the parent's own content checks are what vouch for content. **Required rule:** every
...
`from_state:` check args, WITH the delivery path named: at `scenario_done`, the
...
envelope lives in graph state, invisible to the check dict otherwise, so
the engine must deliver it). A check e.g.
`{type: file_matches_hash, path_from: <state-dir output>, hash_from:
_subcall_envelope.artifact_hashes.<name>}`. Everything else about
evidence rules is unchanged.
...
- `runners.yaml` also carries budgets: `max_subcalls_per_run`,
  ...
  Runner availability/config is checked at START-TIME against the
  snapshot: `validate_recipe` results are
...
- **Gate linkage, bounded**: PreToolUse unlock extends to a
  ...
  **awaiting** run of the policy's recipe in the same project. (Same-user
  ...
  chain RECURSIVELY over grandchildren: requests each
...
- **Child-run origin binding** — the independence fix, without which the
  guarantee is void: a run with `parent_run` set
  ...
  SANCTIONED MCP surface. It does NOT close same-user OS access: on a
  multi-user OS a process environment may be
...
- **Child session posture**: children spawn with the
  ...
  ALLOWLIST: `LOCKSTEP_STATE_DIR` PRESERVED (the
...
- Depth capped by `max_fractal_depth`; budget/runner/depth refusal
  happens ENGINE-SIDE before any graph resume. The
...
- `runs.py`: `parent_run` field + spawn nonce (additive); **RunIndex
  write locking + terminal CAS**: fractal children mean
  ...
  set INCLUDES `done`, else a parent-done cascade rewrites a legitimately
  completed child to `aborted`; the cascade also skips already-terminal
  descendants. "Terminal is never resurrected" must hold in `update()`
  itself, not only in `_reconcile`). The lock is
  ...
  **OS-AGNOSTIC RULE: no platform-specific primitive.** The lock is a portable
  ...
  contention).
- `profile_check.py`: subcall marker is a THIRD interrupt class with its
  own exemption set spelled out: `{step: _subcall, node,
  ...
  pre-resume prediction to be sound; for a
  fractal subcall, the child recipe's `baseline_globs` must cover every
  artifact the parent references via `hash_from:` (the coverage rule);
