...
from lockstep.runtime.effects.descriptors import derive_effect_id, parse_effect_result
from lockstep.runtime.effects.ledger import EffectLedger
from lockstep.runtime.native_models import NativeCoordinate, NativeInterrupt, NativeSnapshot
...
def system(tmp_path):
    ...
    snapshot = NativeSnapshot(
        ...
        pending=(NativeInterrupt(coordinate, {"lockstep_effect": managed_descriptor()}),),
        checkpoint_id="cp-1",
    )
    ...
    coordinator = EffectCoordinator(
        ...
        clock=lambda: NOW,
        owner_factory=lambda: "coordinator",
    )
...
def test_incompatible_or_changed_native_lineage_grants_nothing(system) -> None:
    ...
    runtime.current = replace(
        runtime.current,
        pending=(
            NativeInterrupt(replace(coordinate, checkpoint_id="foreign"), runtime.current.pending[0].value),
        ),
    )
    ...
    assert report.action == "no_effect"
...
def test_provider_scope_result_and_binding_mismatch_cannot_seal(system) -> None:
    ...
    runner.inspect_observations.append(runner.mismatch(runner.terminal(launch, _result(running.effect_id))))
    with pytest.raises(ProviderContractViolation):
        system[0].reconcile("run-1")
    assert system[3].get(running.effect_id).phase == "running"


def test_scope_is_engine_owned_and_inherits_earliest_deadline_without_runner(system) -> None:
    coordinator, runtime, runner, ledger, _store, coordinate = system
    ...
    runtime.current = NativeSnapshot(
        ...
        pending=(
            NativeInterrupt(
                coordinate,
                {"lockstep_effect": scope_descriptor(ancestor_deadline_state_keys=["outer"])},
            ),
        ),
        checkpoint_id="cp-1",
    )
...
def test_post_commit_crash_marks_delivered_from_public_lineage_without_resume(system) -> None:
    from lockstep.runtime.providers.base import TerminalSafetyObservation
