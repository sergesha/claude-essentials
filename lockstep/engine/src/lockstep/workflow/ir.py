...
class FragmentIR:
    ...
    def __init__(self, document: Mapping[str, Any], *, _token: object | None = None) -> None:
        ...
        allowed = {
            "fragment", "state", "nodes", "edges", "loop_limits", "loop_exits"
        }
        ...
        for node in nodes.values():
            ...
            if node["type"] == "passthrough":
                ...
                if (
                    ...
                    raise ValueError(
                        "fragment interrupt requires a protected message and closed keys"
                    )
                if descriptor.get("kind") == "scope" and (
                    ...
                    raise ValueError(
                        "fragment scope result_state_key must equal interrupt resume_key"
                    )
                if (
                ...
                for selector_field in (
                    ...
                    if not isinstance(selectors, (list, tuple)) or any(
                        ...
                        raise ValueError(
                            "fragment scope selectors must reference declared dict state"
                        )
        for edge in edges:
            if (
        ...
        def validate_scalar_tree(value: Any) -> None:
