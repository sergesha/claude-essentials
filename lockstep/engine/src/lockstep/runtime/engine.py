...
class Engine:
    ...
    def recipe_path(self, name: str) -> Path:
        ...
        try:
            return RecipeLoader(self._recipes_dir).resolve(name).path
    ...
    def _child_snapshot_path(self, run_id: str, scenario: str) -> Path:
        ...
        return self._runs_dir() / f"{run_id}.child.{scenario}.recipe.yaml"
    ...
    def _launch(self, recipe: str, vars: dict, project: str,  # noqa: A002 - mirrors start()
        ...
        staging = runs_dir / f".staging-{uuid.uuid4().hex}.recipe.yaml"
        ...
        try:
            ...
            for scenario in scenarios:
                staged = child_staging / f"{scenario}.recipe.yaml"
                if not staged.exists():
                    continue  # missing child: reported by the parent's own check
                child_errors = profile.check_recipe(
            ...
            profile_errors = profile.check_recipe(
    ...
    def _predict_spawn(self, run_id: str, vstatus: str) -> str | None:
        ...
        for e in edges:
            if (e.get("from") == validator
                ...
                target = e.get("to")
                if profile.subcall_node_kind(nodes.get(target), tools) == "spawn":
                    return target
        return None
    ...
    def _subcall_ctx(self, run_id: str, record: RunRecord, spawn_node: str) -> dict:
        ...
        if marker.get("scenario"):
            ...
            prompt = (
                ...
                "scenario_status and scenario_done (tool ids carry the install's "
                "server prefix — commonly mcp__lockstep__ or, under a plugin "
                "install, mcp__plugin_<plugin-name>_lockstep__; match them by their "
                "scenario_* suffix in your tool list. If your harness defers MCP "
                "tools behind a tool-search mechanism, load them first; never substitute "
                f"shell commands for them): call scenario_status with run_id {child_run!r} to "
                "see the parked step, do the work, then report it via scenario_done on "
                "the same run id (scenario_escalate if blocked).\n\n"
                + prompt
            )
